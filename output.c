...
int file_handler (struct system_info_t * system_info, struct monitor_t * monitor, struct poller_t * poller, int rank_list[], double **rank_start_times, double **rank_end_times)
{
    int ret = 0;
    if (monitor->imonitor)
    {
        ...
        {
            if (poli_tags_to_file(system_info, monitor, rank_list, rank_start_times, rank_end_times) != 0)
            {
                ret = (ret || 1);
...
int poli_tags_to_file (struct system_info_t * system_info, struct monitor_t * monitor, int rank_list[], double **rank_start_times, double **rank_end_times)
{
    if (monitor->imonitor)
    {
        FILE *fp = open_file("PoLiMEr_energy-tags", monitor);
        FILE *ft = open_file("PoLiMEr_rank-timings-tags", monitor);
        if (fp == NULL || ft == NULL)
            return 1;
        ...
        fprintf(fp, "\n");
#endif
        fprintf(ft, "Tag Name\tRank\tNode\tMonitor Rank\tStart Time (s)\tEnd Time (s)\tTotal Time (s)\n");
        int tag_num;
        for (tag_num = 0; tag_num < system_info->num_poli_tags; tag_num++)
        {
            ...
            {
                fprintf(ft, "%s\t", tag->tag_name);
                fprintf(ft, "%d\t", rank_list[j]);
                fprintf(ft, "%d\t", tag->monitor_id);
                fprintf(ft, "%d\t", tag->monitor_rank);
                fprintf(ft, "%lf\t", rank_start_times[tag_num][j] - system_info->initial_mpi_wtime);
                fprintf(ft, "%lf\t", rank_end_times[tag_num][j] - system_info->initial_mpi_wtime);
                fprintf(ft, "%lf\n", rank_end_times[tag_num][j] - rank_start_times[tag_num][j]);
            }
        }
