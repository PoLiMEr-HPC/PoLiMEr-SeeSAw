...
double **all_rank_start_times, **all_rank_end_times;
...
static void get_poli_config (void)
{
    ...
    poli_config->pywrapper = getenv("POLIMER_PYTHON_WRAPPER");
...
int poli_integrate_comm(int this_color, int app1_color, int app2_color, int my_app_rank, MPI_Group app_group, MPI_Comm app_comm, int master)
{
#ifdef _POWMGR
    return integrate_communicators(monitor, this_color, my_app_rank, app_group, app_comm, master);
#else
    return 0;
#endif
}
...
static int start_poli_tag_no_sync (char *tag_name)
{
    ...
    new_poli_tag->rank_start_time = get_time();
    memset(new_poli_tag->tag_name, '\0', sizeof(new_poli_tag->tag_name));
    strncpy(new_poli_tag->tag_name, tag_name, strlen(tag_name));
    if (monitor->imonitor)
    {
        ...
        new_poli_tag->start_time = new_poli_tag->rank_start_time;
...
static int end_poli_tag_no_sync (char *tag_name)
{
    ...
    this_poli_tag->rank_end_time = get_time();
...
static int end_existing_poli_tag (struct poli_tag *this_poli_tag)
{
    ...
    if (monitor->imonitor)
    {
        ...
        this_poli_tag->end_time = this_poli_tag->rank_end_time;
...
{
    if (monitor->imonitor && system_info->num_open_tags + system_info->num_closed_tags != 0)
        poli_log(WARNING, monitor, "There are unfinished tags! Attempting to close them...");
        //return 0;
    int chunk_size = 500;
    int chunk_off = 0;
    if (system_info->num_poli_tags < chunk_size)
        chunk_size = system_info->num_poli_tags;

    double *rbuf_start, *rbuf_end;
    if (monitor->imonitor)
    {
        all_rank_start_times = (double **) malloc(system_info->num_poli_tags * sizeof(double *));
        all_rank_end_times = (double **) malloc(system_info->num_poli_tags * sizeof(double *));
        rbuf_start = (double *) malloc (monitor->node_size * chunk_size * sizeof(double));
        rbuf_end = (double *) malloc (monitor->node_size * chunk_size * sizeof(double));
    }
    else
    {
        all_rank_end_times = NULL;
        all_rank_start_times = NULL;
    }

    int tag_num, i, chunk;
    double rst[chunk_size], ret[chunk_size];

    int num_chunks = (int) (system_info->num_poli_tags / chunk_size);
    if (system_info->num_poli_tags % chunk_size != 0)
        num_chunks++;

    for (chunk = 0; chunk < num_chunks; chunk++)
    {
        chunk_off = chunk * chunk_size;
        int chunk_end = chunk_off + chunk_size;
        if (chunk_end > system_info->num_poli_tags)
        {
            chunk_end = system_info->num_poli_tags;
            //chunk_size = system_info->num_poli_tags - chunk_off;
        }

        for (tag_num = chunk_off; tag_num < chunk_end; tag_num++)
        {
            if (monitor->imonitor)
            {
                all_rank_start_times[tag_num] = (double *) malloc(monitor->node_size * sizeof(double));
                all_rank_end_times[tag_num] = (double *) malloc(monitor->node_size * sizeof(double));
            }
            struct poli_tag *this_poli_tag = &system_info->poli_tag_list[tag_num];
            ...
            rst[tag_num - chunk_off] = this_poli_tag->rank_start_time;
            ret[tag_num - chunk_off] = this_poli_tag->rank_end_time;
        }

        MPI_Gather(rst, chunk_size, MPI_DOUBLE, rbuf_start, chunk_size, MPI_DOUBLE, 0, monitor->mynode_comm);
        MPI_Gather(ret, chunk_size, MPI_DOUBLE, rbuf_end, chunk_size, MPI_DOUBLE, 0, monitor->mynode_comm);
        if (monitor->imonitor)
        {
            int j;
            for (j = 0; j < monitor->node_size; j++)
            {
                for (i = chunk_off; i < chunk_end; i++)
                {
                    all_rank_start_times[i][j] = rbuf_start[j * (chunk_end - chunk_off) + (i - chunk_off)];
                    all_rank_end_times[i][j] = rbuf_end[j * (chunk_end - chunk_off) + (i - chunk_off)];
                }
            }
        }
    }
...
int poli_finalize(void)
{
    ...
    app_summary->rank_end_time = get_time();
    ...
    int rank_list[monitor->node_size];
    MPI_Gather(&monitor->world_rank, 1, MPI_INT, &rank_list, 1, MPI_INT, 0, monitor->mynode_comm);
    ...
    if (monitor->imonitor)
    {
        ...
        file_handler(system_info, monitor, poller, rank_list, all_rank_start_times, all_rank_end_times);
        ...
        for (tag_num = 0; tag_num < system_info->num_poli_tags; tag_num++)
        {
            free(all_rank_start_times[tag_num]);
            free(all_rank_end_times[tag_num]);
        }
        free(all_rank_start_times);
        free(all_rank_end_times);
    }
...
/*
    ...
    if (!is_finalized())
    {
        if (monitor->mynode_comm != MPI_COMM_NULL)
            MPI_Comm_free(&monitor->mynode_comm);
        if (monitor->monitors_comm != MPI_COMM_NULL)
            MPI_Comm_free(&monitor->monitors_comm);
#ifdef _POWMGR
        if (monitor->sa_comm != MPI_COMM_NULL)
            MPI_Comm_free(&monitor->sa_comm);
        if (monitor->sa_master_comm != MPI_COMM_NULL)
            MPI_Comm_free(&monitor->sa_master_comm);
#endif
    }
#endif
*/
