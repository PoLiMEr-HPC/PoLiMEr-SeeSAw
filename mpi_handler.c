...
static int get_comm_split_color_hostname (struct monitor_t * monitor)
{
    ...
    char *hostnum;
    if (strncmp(monitor->my_host, "knl", 3) == 0)
    {
        //on JLSE hostnames should be of the form knlxx.ftm.alcf.anl.gov
        char *token;
        token = strtok(hostname, " \t.\n");
        strncpy(monitor->my_host, token, 5);
        hostnum = strtok(monitor->my_host, "knl");
    }
    else if (strncmp(monitor->my_host, "nid", 3) == 0)
    {
        strcpy(monitor->my_host, hostname);
        hostnum = strtok(monitor->my_host, "nid");
    }
