...
static void set_pm_algorithm(int SeeSAw, int SLURMLike, int GEOPMLike, int Bayesian, int Static);
static double get_slack_time (struct monitor_t * monitor, struct pm_log_t *log, double measured_time);
...
static void set_pm_algorithm(int SeeSAw, int SLURMLike, int GEOPMLike, int Static)
{
    power_manager->SeeSAw = SeeSAw;
    power_manager->SLURMLike = SLURMLike;
    power_manager->GEOPMLike = GEOPMLike;

    power_manager->Static = Static;
    if ( !power_manager->SeeSAw && !power_manager->SLURMLike && !power_manager->GEOPMLike && !power_manager->Static)
...
int allocate_power(double power_cap, struct system_info_t * system_info, struct monitor_t * monitor, struct poller_t * poller)
{
    ...
    {
        set_pm_algorithm(0, 1, 0, 0, 0);
        return SLURMLike(power_cap, system_info, monitor, poller);
    }
    if (strcmp(power_manager->pm_algorithm, "GEOPMLike") == 0)
    {
        set_pm_algorithm(0, 0, 1, 0, 0);
        return GEOPMLike(power_cap, system_info, monitor, poller);
    }
    if (strcmp(power_manager->pm_algorithm, "Static") == 0)
    {
        set_pm_algorithm(0, 0, 0, 0, 1);
        return Static(power_cap, system_info, monitor, poller);
    }
    return 0;
}
...
int SeeSAw(double power_cap, struct system_info_t * system_info, struct monitor_t * monitor, struct poller_t * poller)
{
    ...
    double overhead_start = 0;
    double overhead_end = 0;
    if (monitor->imonitor)
    {
        overhead_start = get_time();
    ...
    if (monitor->imonitor && allocate_power && !power_manager->off)
    {
        ...
        get_slack_time(monitor, log, measured_time);
        ...
        log->overhead = overhead_end - overhead_start;
...
int SLURMLike (double power_cap, struct system_info_t * system_info, struct monitor_t * monitor, struct poller_t * poller)
{
    ...
    {   
        ...
        struct pm_log_t *log = &power_manager->logs[power_manager->log_count];
        get_slack_time(monitor, log, sync_time);
...
int GEOPMLike (double power_cap, struct system_info_t * system_info, struct monitor_t * monitor, struct poller_t * poller)
{
    ...
    double measured_time, measured_power;
    ...
    if (power_manager->count)
    {
        ...
        if (monitor->imonitor && allocate_power && !power_manager->off)
            get_past_time_and_power(system_info, &measured_time, &measured_power);
...
{
    int other_size = monitor->monitors_size - monitor->sa_size;
    double slack_time, norm_slack, time_sum;
    MPI_Allreduce(&measured_time, &time_sum, 1, MPI_DOUBLE, MPI_SUM, monitor->monitors_comm);
    double other_measured_time = (time_sum - (measured_time * monitor->sa_size)) / other_size;

    double max_time = measured_time;
    if (other_measured_time > measured_time)
        max_time = other_measured_time;
    slack_time = measured_time - other_measured_time;
    if (slack_time < 0)
        slack_time = slack_time * -1;
    norm_slack = slack_time / max_time;

    log->slack_time = slack_time;
    log->norm_slack = norm_slack;

    return norm_slack;
}

int Static(double power_cap, struct system_info_t * system_info, struct monitor_t * monitor, struct poller_t * poller)
{
    int ret = 0;
    int allocate_power = 0;
    double overhead_start = 0;
    ...
    if (monitor->imonitor && allocate_power && !power_manager->off)
    {
        ...
        power_manager->opt_power_node = power_cap * monitor->sa_size;
        ...
        log->observed_power_all_nodes = max_power_sa_nodes;
        log->overhead = overhead_end - overhead_start;
        log->allocated_power = power_manager->opt_power_node;
        power_manager->log_count++;
    }
...
int integrate_communicators(struct monitor_t * monitor, int this_color, int my_app_rank, MPI_Group app_group, MPI_Comm app_comm, int master)
{
    ...
    poli_log(DEBUG, monitor, "sa_rank: %d\n", monitor->sa_rank);
    if (master)
        monitor->sa_master = 1;
...
void sync_step_end(struct monitor_t * monitor, struct system_info_t * system_info, struct poller_t * poller)
{
    ...
    if (allocate_power && monitor->imonitor && !power_manager->off)
    {
        if (power_manager->new_power_node > 0.3 * system_info->power_info.package_minimum_power && power_manager->new_power_node < 1.3 * system_info->power_info.package_maximum_power)
        {
            ...
            if (power_manager->simulate != 1 && !power_manager->Static)
...
void finalize_SeeSAw (struct system_info_t * system_info, struct monitor_t * monitor)
{
    if (monitor->imonitor)
    {
        ...
        if (power_manager->log_count > 0)
        {
            ...
            fprintf(fp, "Slack (s)\tNormalized Slack\t");
            fprintf(fp, "Time Allocated (s)\tOverhead (s)\n");
            ...
            for (i = 0; i < power_manager->log_count; i++)
            {
                ...
                fprintf(fp, "%lf\t%lf\t", log->slack_time, log->norm_slack);
                fprintf(fp, "%lf\t%lf\n", log->time_allocated, log->overhead);
            }
...
void finalize_SLURMLike (struct system_info_t * system_info, struct monitor_t * monitor)
{
    if (monitor->imonitor)
    {
        ...
        if (power_manager->log_count > 0)
        {
            ...
            fprintf(fp, "Sync Time (s)\tTime Allocated (s)\tOverhead (s)\n");
            ...
            for (i = 0; i < power_manager->log_count; i++)
            {
                ...
                fprintf(fp, "%lf\t%lf\t%lf\n", log->average_time, log->time_allocated, log->overhead);
            }
...
void finalize_GEOPMLike (struct system_info_t * system_info, struct monitor_t * monitor)
{
    if (monitor->imonitor)
    {
        ...
        if (power_manager->log_count > 0)
        {
            ...
            fprintf(fp, "Delta (W)\tObserved Power (W)\tTarget Met\tTime Allocated (s)\tOverhead (s)\n");
            ...
            for (i = 0; i < power_manager->log_count; i++)
            {
                ...
                fprintf(fp, "%lf\t%lf\t%d\t%lf\t%lf\n", log->delta, log->max_observed_power, log->target_met, log->time_allocated, log->overhead);
            }
...
void finalize_Static (struct system_info_t * system_info, struct monitor_t * monitor)
{
    if (monitor->imonitor)
    {
        FILE *fp;
        ...
        if (power_manager->log_count > 0)
        {
            ...
            fprintf(fp, "Count\tCount Allocation\tSim Timestep\tRank\t");
            fprintf(fp, "Slack (s)\tNormalized Slack\t");
            fprintf(fp, "Allocated Power (W)\tNew Power Node (W)\t");
            fprintf(fp, "Measured Power (W)\t");
            fprintf(fp, "Measured Time (s)\t");
            fprintf(fp, "Time Allocated (s)\tOverhead (s)\n");
            ...
            for (i = 0; i < power_manager->log_count; i++)
            {
                ...
                fprintf(fp, "%d\t%d\t%d\t%d\t", log->pm_count, log->palloc_count, log->timestep, log->world_rank);
                fprintf(fp, "%lf\t%lf\t", log->slack_time, log->norm_slack);
                fprintf(fp, "%lf\t%lf\t", log->allocated_power, log->new_power_per_node);
                fprintf(fp, "%lf\t", log->max_observed_power);
                fprintf(fp, "%lf\t", log->average_time);
                fprintf(fp, "%lf\t%lf\n", log->time_allocated, log->overhead);
            }
...
void finalize_power_manager (struct system_info_t * system_info, struct monitor_t * monitor)
{
    ...
    else if (power_manager->Static)
        finalize_Static(system_info, monitor);
